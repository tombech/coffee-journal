...
def create_app(test_config=None):
    ...
    try:
        print(f"🔄 Initializing migration system for data directory: {app.config['DATA_DIR']}")

        # Check if data directory exists
        if not os.path.exists(app.config['DATA_DIR']):
            print(f"⚠️  Data directory does not exist: {app.config['DATA_DIR']}")
        else:
            print(f"✅ Data directory found: {app.config['DATA_DIR']}")
        ...
        print(f"📊 Schema version: {current_schema}, Data version: {current_data}")
        ...
        if migration_manager.needs_migration():
            print(f"🚀 Data migration required: {current_data} -> {current_schema}")
            ...
            if migration_key in migration_manager.migrations:
                print(f"✅ Migration found: {migration_key}")
            else:
                print(f"❌ Migration not found: {migration_key}")

            print("💾 Creating backup and running migrations...")
            ...
            print("✅ Data schema is up to date - no migration needed")
    except Exception as e:
        print(f"❌ Error during migration check: {str(e)}")
        print("Application will continue with potentially outdated data schema")
